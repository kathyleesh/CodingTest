...
'''
# 지름길의 개수 N과 고속도로의 길이 D를 입력받는다.
N, D = map(int, input().split())
# 지름길의 정보를 받을 배열
arr = []

# 지름길의 정보를 받아 split 한다음 list로 만든다.
for i in range(N):
    # 지름길의 시작 위치, 도착 위치, 지름길의 길이를 N개 만큼 입력 받는다.
    start, end, distance = map(int, input().split())
    arr.append([start, end, distance])
    # list를 시작 위치 기준으로 오름차순 정렬을 한다.
    arr.sort(key=lambda x: x[0])

# 전체 길이를 탐색할 수 있도록 list를 만든다.
search = [i for i in range(D+1)]

# 전체 길이를 반복을 통해 탐색한다.
for i in range(D+1):
    for start, end, distance in arr:
        # 정석대로 고속도로로 갔을 때의 길이와 지름길로 갔을 때의 길이 중 최소 값을 저장한다.
        search[i] = min(search[i], search[i-1]+1)
        # 탐색 중 지름길의 시작 위치가 존재하고 전체 가야하는 길이 전에 지름길의 도착 위치가 존재하며 지름길의 길이가 고속도로로 갔을 때보다 빠르면 지름길로 가게끔 저장한다.
